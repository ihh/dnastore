...
df.l4.ldpc <- read.table(file="len4.wat64.1.ldpc.subs.tab");
...
df$Code=factor(df$group,labels=c("DNASTORE(4)","MIXRADAR(2) + DNASTORE(4)","LDPC(2k,1k) + WATMARK(64) + DNASTORE(4)","HAMMING(7,4) + DNASTORE(4)"));
