...
subplot <- ggplot(df,aes(x=SubProb,y=MeanEditsPerBit,color=Code)) + geom_errorbar(aes(ymin=MeanEditsPerBit-2*StDevEditsPerBit,ymax=MeanEditsPerBit+2*StDevEditsPerBit)) + geom_point(aes(shape=Code),size=2) + ylab("Edits per bit") + xlab("Substitution probability per base") + ggtitle("Effect on decoding accuracy of point substitutions") + theme(legend.position=c(.25,.85)) + scale_x_log10();
