...
These synchronization codes are certainly relevant to error-resistant DNA coding,
though error models typically used to motivate such codes
(such as the binary erasure channel or single-bit deletion channels)
may rather poorly characterize the nature of such errors in DNA:
biological indels are very often burst errors,
affecting multiple consecutive nucleotides in a single event.
(Burst substitution errors are not uncommon either.)
...
However, there are hidden assumptions in this reasoning, specifically that it is worth investing a particular amount on synthesis in order to improve sequencing by another particular amount.
