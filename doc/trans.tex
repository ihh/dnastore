...
Goldman {\em et al} \cite{GoldmanEtAl2013} synthesized DNA using a ternary (radix-3) code that avoids repeated nucleotides
...
proposed several codes and analyzing their combinatoric properties \cite{MilenkovicEtAl2014,MilenkovicEtAl2015,GabrysEtAl2015,MilenkovicEtAl2016}
and devised a random-access rewritable filesystem \cite{YazdiEtAl2015}.
In work not specifically oriented to DNA storage,
Davey and MacKay describe error-correcting ``watermark'' codes that are resistant to
...
so we might want to pack in more information---and yet still (perhaps) reserve certain motifs,
...
(an assumption which might motivate the ternary code),
it may also be desirable to avoid other motifs prone to sequencing error, such as repeated dinucleotides, trinucleotides or longer repeats.
...
Error-correcting codes (for example, Hamming codes that introduce parity bits, or watermark codes for synchronization in the presence of insertions and deletions)
...
\secref{Watermarks} describes a transducer that introduces a watermark for synchronization.
...
The watermark was introduced by Davey and MacKay as a frame-preserving signal that can be used as an ``inner code'' to correct for insertion and deletion errors
...
Using transducers we can introduce watermark-type synchronization in several ways:
(i) we can construct an $M$-state cyclic transducer that
introduces one of the control signals from \secref{DeBruijnTransducer}
(i.e. a reserved word) into the output DNA after every $M$ input bits;
(ii) we can give each of the $M$ states a unique pseudorandom signature for radix conversion,
so that every (binary) input digits is mapped uniquely to a single (binary, ternary or quaternary) output digit,
but this mapping is performed slightly differently by each of the $M$ states;
(iii) for additional synchronization we can have the transducer interleave extra (fixed-value) bits into the encoded sequence,
for example after every $k$ states.

When using watermark codes, we first encode the message using an LDPC code,
which we then apply to the decoded sequence at the other end.
The block length of the LDPC code can be matched to the block length of the watermark code
...
the watermark synchronization codes of \secref{Watermarks};
...
The results of these computational experiments are summarized in \figref{DupPlot}, \figref{SubPlot} and \figref{DelPlot}.
% WRITE ME

% Plot: per-bit edit distance vs per-base prob of substitutions, duplications, single-base deletions, multi-base deletions
...
inserting watermarks or metadata, adding parity bits, converting binary to mixed-radix, generating repeat-free DNA,
or modeling errors, or for other tasks that we have not thought to include.

We have not addressed the higher-level organization of the DNA ``filesystem'',
which has been explored in work by others \cite{YazdiEtAl2015,BornholtEtAl2016}.
...
\easyfig{dupplot.ps}{width=\textwidth}{DupPlot}{
  Effect of simulated duplications on decoder accuracy for two different error models.
  ...
  MIXRADAR(2)+DNASTORE(4) with 2 bases of error-model context, MIXRADAR(2)+DNASTORE(8) with 4 bases of error-model context
  ...
10 repetitions,
1 kilobyte random sequence
...
Up to four nucleotides randomly duplicated
...
...
1 kilobyte random sequence
...
Transition/transversion ratio of 10
...
Levenshtein edit distance
}

\easyfig{delplot.ps}{width=\textwidth}{DelPlot}{
  Effect of simulated deletions on decoder accuracy for three different codes.
...
10 repetitions,
1 kilobyte random sequence
...
