...
(so $\ntrans{\nuca} = \nucg$, $\ntrans{\nucc}=\nuct$, etc.)
and let $\ntranscomp{x}$ denote the complement of $\ntrans{x}$.
...
\begin{itemize}
\item There is a state $S_A(\emptystring,\emptystring)$ which is the initial state
 %
\item For every $k:1 \leq k < \contextlen$ and every $k$-mer $\inseqnext \in \outalph^k$
  there is a state $S_B(\emptystring,\inseqnext)$
  %
\item For every $j:0 \leq j \leq \contextlen$, every $j$-mer $\inseqpast \in \outalph^j$ and every $\contextlen$-mer $\inseqnext \in \outalph^\contextlen$
  there is a state $S_C(\inseqpast,\inseqnext)$,
  $\contextlen$ states $\{ R_C(\inseqpast,\inseqnext,i): 1 \leq i \leq \contextlen \}$ and
  $2j$ states $\{ T_C(\inseqpast,\inseqnext,i), F_C(\inseqpast,\inseqnext,i): 1 \leq i \leq j \}$
  %
\item For every pair of $\contextlen$-mers $\inseqpast, \inseqnext \in \outalph^\contextlen$
  there is a state $S_D(\inseqpast,\inseqnext)$ and
  $3\contextlen$ states $\{ T_D(\inseqpast,\inseqnext,i), F_D(\inseqpast,\inseqnext,i), R_D(\inseqpast,\inseqnext,i): 1 \leq i \leq \contextlen \}$
  %
\item For every $k:0 \leq k < \contextlen$, every $\contextlen$-mer $\inseqpast \in \outalph^\contextlen$ and every $k$-mer $\inseqnext \in \outalph^k$
  there is a state $S_E(\inseqpast,\inseqnext)$,
  $2\contextlen$ states $\{ T_E(\inseqpast,\inseqnext,i), F_E(\inseqpast,\inseqnext,i): 1 \leq i \leq \contextlen \}$ and 
  $k$ states $\{ R_E(\inseqpast,\inseqnext,i): 1 \leq i \leq k \}$ 
  %
\item There is a state $S_F(\emptystring,\emptystring)$ which is the final state
\end{itemize}
These states have the following significance:
\begin{itemize}
  \item $S_B$ states load input symbols into the future context queue
  \item $S_C$ states have fully loaded future context queues. They continue to load input symbols, but also start emitting output symbols and shifting input symbols to the past context queue
  \item $S_D$ states have fully loaded past and future context queues. They load input symbols, shift input symbols from future to past context queues, drop input symbols off the back of the past context queue, and emit output symbols
  \item $S_E$ states have fully loaded past context queues, but emptying future context queues. No future input symbols are loaded at this point. They shift input symbols from future to past context queues, drop input symbols off the back of the past context queue, and emit output symbols
  \item $T_x$ states are used for tandem duplications
  \item $F_x$ states are used for forward inverse duplications
  \item $R_x$ states are used for reverse inverse duplications
...
% where \submat(i,j) = \left{ identity... transitions... transversions...
