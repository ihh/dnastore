...
Our implementation of these algorithms includes a few extra codes, such as a synchronization code
that transmits a reserved control word (as described in \secref{DeBruijnTransducer})
after every $N$ bits. The modular nature of transducers makes it straightforward to incorporate
additional such variations into the coding framework, with the caveat that each such additional
component machine multiplies the state and transition sets of the composite transducer
and that the cost of Viterbi
decoding scales in memory with the number of states and in time with the number of transitions.

While some error-correcting codes, such as short Hamming codes, can be conveniently represented
as state machines with exhaustively enumerable state spaces (as we have done here),
more sophisticated codes---such as convolutional
codes---generally have too many possibilities to enumerate efficiently.
However, these codes can be represented as graphical models \cite{FreyMackay98,MurphyEtAl1999};
likewise HMMs can be represented as trellis graphical models,
which may offer a common framework for decoding such codes.
