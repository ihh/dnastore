...
However, we can also use the extra information in the trinary and quaternary bits for error-correction purposes:
...
so that every (binary) input digits is mapped uniquely to a single (binary, trinary or quaternary) output digit,
...
By default, DNASTORE uses the naive binary-to-trinary conversion of
...
Composite state machines incorporating other stages
(e.g. more sophisticated radix conversion, or error correcting codes)
can be incorporated in this way.
...
In common with Gray Codes \cite{GrayCodes}, motivation for our code is error-avoidance, not error-correction.
However does have some error-correcting properties.
If noise from DNA degradation/sequencing is unavoidable,
it would be necessary to upgrade error-correcting features
...
\easyfig{subplot.ps}{width=\textwidth}{SubPlot}{
  Effect of simulated substitution errors on decoder accuracy for three different codes.
...
10 repetitions,
1 kilobyte random sequence
...
