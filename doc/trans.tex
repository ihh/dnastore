...
Left of middle row (b):
Transducer $\transternid$ implements ``trinary identity'', copying a trinary input string to its output.
Center of middle row (c):
Transducer $\transbinerase$ erases a single binary zero and outputs the empty string.
Right of middle row (d):
Transducer $\transternid \transconcat \transbinerase$ is the concatenation of the first two.
Bottom row (e):
Transducer $\transternid \transconcat \transdivthree$
is a concatentation of the ``trinary identity'' transducer (b)
...
\caption{
  ...
  Central column (b,e): codes in which there is one reserved control words, which only ever appears at the start of the encoded DNA sequence.
