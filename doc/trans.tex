...
Given transducers
 $\trr = (\inalph, \somealph, \states_R, \transitions_R, \initstate_R, \finalstate_R)$ and
 $\trs = (\somealph, \outalph, \states_S, \transitions_S, \initstate_S, \finalstate_S)$
where $\trr$'s output alphabet is the same as $\trs$'s input alphabet,
we can readily find a composite transducer
 $\trt = \trr \transcomp \trs = (\inalph, \outalph, \states_T, \transitions_T, \initstate_T, \finalstate_T)$
...
\item The initial $\trt$-state $\initstate_T=(\initstate_R,\initstate_S)$ pairs the initial states of $\trr$ and $\trs$.
\item The final $\trt$-state $\finalstate_T=(\finalstate_R,\finalstate_S)$ pairs the final states of $\trr$ and $\trs$.
\item $\trt$-transitions
...
is concatenation.
Suppose
 $\tra = (\inalph, \outalph, \states_A, \transitions_A, \initstate_A, \finalstate_A)$ and
 $\trb = (\inalph, \outalph, \states_B, \transitions_B, \initstate_B, \finalstate_B)$
