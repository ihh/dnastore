...
\newcommand\titlestring{Systematic repeat-avoiding codes for DNA storage}
...
This makes it an appealing prospect for long-term archival storage \cite{ChurchEtAl2012}.
However, since DNA synthesis, sequencing, and (if applicable) replication are prone to certain kinds of error,
it is advisable to treat the storage of information in DNA like other noisy channel coding problems.
...
Goldman {\em et al} \cite{GoldmanEtAl2013} synthesized DNA using a trinary (radix-3) code that avoids repeated nucleotides, synthesizing to fourfold redundancy for error correction.
...
The purpose of this paper is to show that several of the low-level tasks in coding information into DNA
...
(the assumption motivating the trinary code),
...
We may also wish to introduce error-correcting codes analogous to Hamming codes \cite{Mackay2003}, turbo codes \cite{FreyMackay98,MurphyEtAl1999}, or Golay codes \cite{GuptaEtAl2015}.
...
\figref{DNAStore} shows repeat-avoiding transducers that encode mixed-radix sequences,
...
which is quite close to the theoretical optimum of $\log_2(3) \simeq 1.585$ bits per trit.
...
We can send an extra padding bit, or we can introduce a ``flush'' symbol,
but we have to be careful that the decoder does not become unsynchronized with the encoder.

As a more robust solution to the problem of efficiently converting a binary sequence into a mixed-radix sequence
we construct a block code inspired by the arithmetic coding algorithm \cite{Rissanen1976,Mackay2003}.
...
It is common to include an end-of-file character `\$' in the symbol alphabet
and require that the last message symbol is this character.
...
We construct a transducer that converts an input word into a mixed-radix sequence
with all possible combinations of binary, trinary and quaternary radix at every available position, as follows:
\begin{itemize}
\item First construct the prefix tree of the input words, assign a state to every node and an input-labeled transition to every correspondingly labeled edge.
  The root of the prefix tree serves as the initial and final state.
\item Let $S$ be the list of states corresponding to the leaves of the prefix tree.
  For $s \in S$, let the {\em input interval} $[A(s),B(s))$ be the subinterval of $[0,1)$ required to encode the associated input word.
    \item Also for each state define the {\em output prefix}, $O(s)$, and the {\em output interval} $[D(s),E(s))$, the subinterval that has been encoded by the output prefix.
      For the states $s \in S$, define $O(s) = \emptystring$ and $[D(s),E(s)) = [0,1)$ since no output symbols have been emitted yet.
        \item While $S$ is nonempty, then for the first $s \in S$:
        \begin{itemize}
          \item If the output interval is not fully enclosed by the input interval, that is if not ($D(s) \geq A(s)$ and $E(s) \leq B(s)$), then:
            \begin{itemize}
              \item Let $m = A(s) + \frac{1}{2}(B(s) - A(s))$ be the midpoint of the input interval and let $w = E(s) - D(s)$ be the width of the output interval.
            \item For each radix $R$ from 2 to 4:
              \begin{itemize}
              \item Find the integer $r$ such that if $d(r)=D(s)+wr/R$ then $d(r) \leq m < d(r+1)$.
              \item Create a new state $t$ and define $A(t)=A(s)$, $B(t)=B(s)$, $D(t)=d(r)$, $E(t)=d(r+1)$ and $O(t) = O(s) \cdot r_R$.
              \item Add a transition from $s$ to $t$ with output label $r_R$.
              \item Add $t$ to $S$.
              \end{itemize}
            \end{itemize}
          \item Delete $s$ from $S$.
        \end{itemize}
      \item If any states have a unique output prefix $O(s)$, then remove all their outgoing transitions and all descendant states reachable via those transitions.
      \item Merge all states whose sets of possible output sequences are identical. These are easily identified since each state's output sequence set is generated by the subtree of states accessible from that state: there are no cycles in the transition graph.
        A string encoding of a state's accessible subtree is sufficient to identify its output set.
      \item Form the Kleene closure of the constructed transducer by adding a unit-weight, $\emptystring$-labeled transition from every state without outgoing transitions back to the initial state.
\end{itemize}

\figref{Arithmetic} shows a transducer that was designed for the example alphabet (2-digit codewords, $\nu=1/10$).
...
\easyfig{eof2.ps}{width=\textwidth}{Arithmetic}{
  A transducer 
  that converts a binary codeword of length 0, 1 or 2 bits
  into a mixed-radix sequence.
  Dotted lines show transitions that input a bit or an end-of-file symbol (`\$').
  ...
  This code was generated using the automated procedure of \secref{Arithmetic}
  with parameter $\nu$ (the probability of the end-of-file symbol) equal to 1/10.
}
