...
we construct a block code inspired by arithmetic coding \cite{Mackay2003}.
...
We take this idea and apply it to mixed-radix sequences.
...
\item Let $S$ be the list of states corresponding to the leaves of the prefix tree.
  For $s \in S$, let the {\em input interval} $[A(s),B(s))$ be the subinterval of $[0,1)$ required to encode the associated message.
    \item Also for each state define the {\em output interval} $[D(s),E(s))$, the subinterval that has been encoded by output symbols so far.
      For the states $s \in S$, define $[D(s),E(s)) = [0,1)$ since no output symbols have been emitted yet.
        ...
        \begin{itemize}
          \item If the output interval is not fully enclosed by the input interval, that is if not ($D(s) \geq A(s)$ and $E(s) \leq B(s)$), then:
            ...
            \item For each radix $R$ from 2 to 4:
              ...
              \item Create a new state $t$ and define $A(t)=A(s)$, $B(t)=B(s)$, $D(t)=d(r)$ and $E(t)=d(r+1)$.
