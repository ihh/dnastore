...
\newpage
\dotfig{hamming}{width=\textwidth}{HammingTransducer}{
...
\newpage
\dotfig{naive}{width=\textwidth}{NaiveDNATransducer}{
A transducer that converts a binary input string into DNA.
The initial state is S and the final state is END.}

\newpage
\begin{figure}
\includedot{divisionby3}{width=\textwidth}
\includedot{binary2trinary}{width=\textwidth}
\caption{
...
Bottom:
A concatenation of the ``division by three'' transducer (the $0$, $1$, $10$ and END states)
with an identity transducer (the $S$ state)
that copies trinary digits from its input to its output.
By composing a series of $N$ transducers of this form, one can convert a binary number
into trinary, up to $N$ trinary digits \cite{MartinsFerreira2012}.
}
\end{figure}

\newpage
\dotfig{converter}{width=\textwidth}{Converter}{
