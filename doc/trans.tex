...
adopting the further convention that edges are labeled $\inlab{\tt /}\outlab$
to show both input and output labels, with empty labels ($\emptystring$) omitted
...
\easyfig{hamming.png}{}{HammingTransducer}{
...
\easyfig{naive.png}{}{NaiveDNATransducer}{
A transducer that converts a binary input string into DNA.
The initial state is S and the final state is END.}

\easyfig{divisionby3.png}{width=\textwidth}{NaiveDNATransducer}{
A transducer that implements the operation ``division by three''.
The machine accepts a binary divisor as its input sequence, most significant bit first.
It outputs the quotient (as a similar binary sequence) followed by the remainder (as a trinary digit).
Binary digits are written as \{ \bit{0}, \bit{1} \} and trinary digits as \{ \trit{0}, \trit{1}, \trit{2} \}.
By chaining together multiple transducers of this form, one can convert a binary number
into trinary, up to a fixed number of trinary digits,
with no loss of information \cite{MartinsFerreira2012}.
}

\easyfig{converter.png}{height=.8\textheight}{Converter}{
A transducer that converts an input sequence of binary digits (\bit{0},\bit{1}),
...
(Technically state C can be merged with state S, but is shown separately for layout reasons.)
The conversion is wasteful in that trinary digits (trits) on average encode
