...
Left:
Transducer $\transbintoquat$ (a) converts a binary input string (presented least-significant digit first) into a quaternary input string.
...
Middle:
Transducer $\transquattodna$ (b) converts a quaternary input string into a DNA string.
Right:
Transducer $\transbintoquat \transcomp \transquattodna$ (c)
...
\\
(b) \includedot{trinaryid}{width=.2\textwidth}
&
(c) \includedot{binaryeraser}{width=.15\textwidth}
&
(d) \includedot{trinaryonly}{width=.45\textwidth}
\\
\multicolumn{3}{c}{}
(e) \includedot{binary2trinary}{width=\textwidth}
\end{tabular}
\caption{
\figlabel{DivisionByThree}
Top row:
Transducer $\transdivthree$ (a) implements the operation ``division by three''.
...
The left machine $\transternid$ (b) implements ``trinary identity'', copying a trinary input string to its output.
The middle machine $\transbinerase$ (c) erases a single binary zero and outputs the empty string.
The right machine $\transternid \transconcat \transbinerase$ (d) is the concatenation of the first two.
Bottom row:
Transducer $\transternid \transconcat \transdivthree$ (e)
is a concatentation of the ``trinary identity'' transducer $\transternid$ (b)
with the ``division by three'' transducer $\transdivthree$ (a).
...
\figlabel{Converter}
Left:
Transducer $\transmixed$ (a) converts an input sequence of binary digits ($\bit{0},\bit{1}$),
...
Right:
Transducer $\transflush$ (b) copies input bits to the output, sending a flush symbol after every 8-bit byte.
...
\caption{
  ...
  from which vertices are duplicated, deleted and added to arrive at a machine with the designated properties.
  ...
  Middle column (b,e): codes in which there is one reserved control words, which only ever appears at the start of the encoded DNA sequence.
  Right-hand column (c,f): codes in which there is one reserved control words, which only ever appears at the start and end of the encoded DNA sequence.
  The leftmost machine in the top row (a) is equivalent to $\transbintoquat \transcomp \transquattodna$ of \figref{NaiveDNATransducer}(c),
  while the leftmost machine in the bottom row (d) is equivalent to the code of \cite{GoldmanEtAl2013}.
  {\bf Key:}
